...
export async function POST(request: NextRequest) {
	try {
		...
		const webdavUserFolder =
			meResult.me.username?.trim() || userDocumentId
		...
		const storageKey = buildPhotoboxStorageKey(
			webdavUserFolder,
			collectionId,
			image.name,
		)

		console.log(`[PhotoBox API] Uploading raw file via WebDAV: ${storageKey} (${originalBuffer.length} bytes)`)

		await putPhotoboxRawFile(storageKey, originalBuffer, originalImage.type || 'image/jpeg')
